...
const isOffchainSchemaAValidUrl = (
  offchainSchema: string | undefined
): offchainSchema is string => {
  return (
    typeof offchainSchema === "string" &&
    validateURLSafe(offchainSchema, "offchainSchema") &&
    offchainSchema.indexOf("{id}") >= 0
  );
};

export const decodeOldSchemaCollection = async (
  collectionId: number,
  properties: PropertiesArray,
  options: Required<DecodingImageLinkOptions>
): Promise<DecodingResult<UniqueCollectionSchemaDecoded>> => {
  ...
  const constOnchainSchema: string | undefined =
    propObj._old_constOnChainSchema;
  const schemaVersion: string | undefined = propObj._old_schemaVersion;
  const variableOnchainSchema: string | undefined =
    propObj._old_variableOnChainSchema;
  ...
  if (
    parsedVariableOnchainSchema &&
    typeof parsedVariableOnchainSchema === "object" &&
    typeof parsedVariableOnchainSchema.collectionCover === "string"
  ) {
    schema.coverPicture.ipfsCid = parsedVariableOnchainSchema.collectionCover;
    delete schema.coverPicture.url;
    schema.coverPicture.fullUrl = imageUrlTemplate.replace(
      "{infix}",
      parsedVariableOnchainSchema.collectionCover
    );
  ...
  for (const field of NFTMeta.fieldsArray) {
    ...
    const options =
      ...
      root.lookupEnum(field.type).options;
...
export const decodeOldSchemaToken = async (
  collectionId: number,
  tokenId: number,
  rawToken: { owner: any; properties: any[] },
  ...
  if (!constOnchainSchema) {
    return {
      ...
      error: new ValidationError(
        `collection doesn't contain _old_constOnChainSchema field`
      ),
    };
  }
  ...
  if (!rawToken) {
    return {
      result: null,
      error: new ValidationError(
        `parsing token with old schema: no token passed`
      ),
    };
  }
  ...
  const constDataProp = parsedToken.properties.find(
    ({ key }) => key === "_old_constData"
  );
  if (!constDataProp) {
    return {
      result: null,
  ...
  try {
    ...
    tokenDecodedWithRawValues = getEntries(tokenDecoded).map(
      ([name, rawValue], index) => ({
        name,
        rawValue: rawValue,
  ...
  for (const entry of tokenDecodedWithRawValues) {
    ...
    const schemaAttr = getEntries(schema.attributesSchema!).find(
      ([attrId, attrValue]) => {
    ...
    tokenAttributesResult[index] = {
      name: { _: name },
      type:
        field.type === "number" ? AttributeType.float : AttributeType.string,
      value,
      isArray,
      isEnum,
  ...
  if (
    parsedToken.owner.Ethereum &&
    Address.is.nestingAddress(parsedToken.owner.Ethereum)
  ) {
    decodedToken.nestingParentToken = Address.nesting.addressToIds(
      parsedToken.owner.Ethereum
    );
  }
