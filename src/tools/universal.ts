...
export const parseImageLinkOptions = (
  ...
  const dummyImageFullUrl =
    typeof options?.dummyImageFullUrl === "string"
...
export const universallyDecodeCollectionSchema = async (
  collectionId: number,
  properties: PropertiesArray,
  flags: CollectionFlags,
  options?: DecodingImageLinkOptions
): Promise<DecodingResult<UniqueCollectionSchemaDecoded>> => {
  const schemaNameProp =
    properties.find(({ key }) => key === "schemaName")?.value || null;
  const schemaName =
    typeof schemaNameProp === "string"
      ? safeJsonParseStringOrHexString<string>(schemaNameProp)
      : null;
  const isOldSchema = !!properties.find(
    ({ key }) => key === "_old_schemaVersion"
  );
  const isERC721Metadata = flags.erc721metadata === true;

  if (isOldSchema) {
    const imageLinkOptions = parseImageLinkOptions(options);
    return await oldSchema.decodeOldSchemaCollection(
      collectionId,
      properties,
      imageLinkOptions
    );
  } else if (schemaName === COLLECTION_SCHEMA_NAME.unique) {
    return await collection.decodeUniqueCollectionFromProperties(
      collectionId,
      properties
    );
  } else if (isERC721Metadata) {
    return collection.decodeUniqueCollectionFromERC721Metadata(
      collectionId,
      properties
    );
  }
...
export const universallyDecodeToken = async (
  collectionId: number,
  tokenId: number,
  ...
  if (!schema) {
    return {
      ...
      error: new ValidationError(
        "unable to parse: collection schema was not provided"
      ),
    };
  }
  const humanizedToken: HumanizedNftToken =
    rawToken.toHuman() as HumanizedNftToken;

  if (schema.schemaName === COLLECTION_SCHEMA_NAME.unique) {
    return await token.decodeTokenFromProperties(
      collectionId,
      tokenId,
      humanizedToken,
      schema
    ...
    const imageLinkOptions = parseImageLinkOptions(options);
    return await oldSchema.decodeOldSchemaToken(
      collectionId,
      tokenId,
      rawToken,
      schema,
  ...
  return {
    result: null,
    error: new ValidationError(
      `unable to parse: collection schemaName is unknown (passed ${schema.schemaName}`
    ),
  };
};
