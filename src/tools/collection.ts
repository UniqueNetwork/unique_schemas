...
import {
  validateCollectionTokenPropertyPermissions,
  validateUniqueCollectionSchema,
} from "./validators";
import {
  PropertiesArray,
...
export const encodeCollectionSchemaToProperties = (
...
export const unpackCollectionSchemaFromProperties = (
  properties: PropertiesArray
): any => {
  return converters2Layers.propertiesToObject(properties) as any;
};

export const decodeUniqueCollectionFromProperties = async (
  collectionId: number,
  properties: PropertiesArray
): Promise<DecodingResult<UniqueCollectionSchemaDecoded>> => {
  try {
    const unpackedSchema: UniqueCollectionSchemaDecoded =
      unpackCollectionSchemaFromProperties(properties);
    ...
    if (unpackedSchema.coverPicture) {
      unpackedSchema.coverPicture = decodeTokenUrlOrInfixOrCidWithHashField(
        unpackedSchema.coverPicture,
        unpackedSchema.image
      );
    }
    if (unpackedSchema.coverPicturePreview) {
      unpackedSchema.coverPicturePreview =
        decodeTokenUrlOrInfixOrCidWithHashField(
          unpackedSchema.coverPicturePreview,
          unpackedSchema.image
        );
...
export const decodeUniqueCollectionFromERC721Metadata = (
  collectionId: number,
  properties: PropertiesArray
): DecodingResult<UniqueCollectionSchemaDecoded> => {
  try {
...
const generateDefaultTPPObjectForKey = (
  key: string
): TokenPropertyPermissionObject => ({
  key,
...
const generateDefaultTPPsForInfixOrUrlOrCidAndHashObject = (
  permissions: CollectionTokenPropertyPermissions,
  prefix: string
) => {
...
export const generateTokenPropertyPermissionsFromCollectionSchema = (
  schema: UniqueCollectionSchemaToCreate,
  options?: ICollectionSchemaToTokenPropertyPermissionsOptions
): CollectionTokenPropertyPermissions => {
  ...
  if (options?.overwriteTPPs) {
    ...
    for (const tpp of overwriteTPPs) {
      const index = permissions.findIndex(
        (permission) => permission.key === tpp.key
      );
      if (index < 0) {
